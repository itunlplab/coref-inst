...
def mwt_serialize(words):
    for w in words:
        if type(w) is list:
            idx = words.index(w)
            for item in w[::-1]: words.insert(idx, item)
            words.remove(w)
...
def match_mentions(mention_words, words):
    ...
    for gold_words, gold_head in mention_words:
        ...
        if gold_words[-1].multiword_token:
            gold_end_address = str(gold_words[-1].multiword_token.words[-1].address)
        ...
        # End match
        elif gold_end_address == found_end:
    ...
    if alternatives:
        if len(alternatives) == 1:
            ...
            return min(alternatives, key=lambda i: abs(i[0][0].ord - words[0].ord)) # first word dist
    
    raise Exception(f"Not matched {words}")

def map_to_udapi(doc, clusters_independent, prev_ent_counter, mention_words):
    udapi_words = list(doc.nodes_and_empty)
    mention_words = [([[j for j in list(doc.nodes_and_empty) if j.address() == w][0] for w in i[0]], [j for j in list(doc.nodes_and_empty) if j.address() == i[1]][0]) for i in mention_words]
...
class Mapper:
    def __init__(self, doc, llm_output_windows):
        ...
        self.llm_windows = [(window1, window2) for window1, window2 in llm_output_windows]
        ...
        for node in self.doc.nodes_and_empty:
            if node.multiword_token:
                if last_mwt == node.multiword_token: continue
    ...
    def map_outputs(self, prev_ent_counter, gold_conllu_fp):
        ...
        for row in self.llm_windows:
            self.udapi_word_forms, clusters_1, start_w_1 = self.create_clusters(self.udapi_word_forms, row[0], start_w_1)
            udapi_word_forms_2 = copy.copy(self.udapi_word_forms)
            start_w_2 = copy.copy(start_w_1)
            udapi_word_forms_2, clusters_2, start_w_2 = self.create_clusters(udapi_word_forms_2, row[1], start_w_2)
            self.window_clusters.append((clusters_1, clusters_2))

        self.merge_clusters()

        # Remove entity misc and re-read the document
        mention_words = [([w.address() for w in i.words], i.head.address()) for i in self.doc.coref_mentions]
        
        doc_cpy = udapi.Document(gold_conllu_fp, encoding="utf-8")
        udapi.block.corefud.removemisc.RemoveMisc(attrnames="Entity,SplitAnte,Bridge").apply_on_document(doc_cpy)
        udapi.block.write.conllu.Conllu(files=['dev_help.conllu']).apply_on_document(doc_cpy)
        doc = udapi.Document('dev_help.conllu')

        del self.doc
        self.doc = map_to_udapi(doc, self.clusters, prev_ent_counter, mention_words)
    ...
    def merge_clusters(self):
        ...
        for before, after in self.window_clusters:
            ...
            for k, v in before.items():
                if not k in self.clusters: raise f"WINDOWS ARE NOT SAME {k}, {v}"
    ...
    def create_clusters(self, udapi_word_forms, window, starting_word_order):
        ...
        while window:
            window = window.strip()
            if udapi_word_forms: form = udapi_word_forms[0]
            else: form = 'None' # There are still some mentions at the end
            ...
            try:
                ...
                if window.startswith('<m>'):
                    ...
                    window = window[len('</m>#'):]
                    cluster_id = re.match(r'^\d+', window).group() if re.match(r'^\d+', window) else None
                    window = window[len(cluster_id):]
                    start_id = mention_starts.pop()
                    
                    # ill-contioned mentions
                    if (start_id, w_id) in new_clusters.keys():
...
def write_docs_to_files(dir, filename):
    ...
    with open(filename, 'r') as f:
        docs = re.split(r'(?=# newdoc)', open(filename, 'r').read())[1:]
        
        for text in docs:
            doc_id = text.split('\n')[0].split('=')[1].strip()
            doc_order.append(doc_id)
            doc_id = doc_id.replace('/', '%2')
            with open(f'{dir}/{doc_id}+{os.path.basename(filename)}', 'w') as doc_file:
                doc_file.write(''.join(text))

    return doc_order

def main(filename, output_file, raw_windows):
    s = 'mapper/'

    lang = raw_windows[0]['lang']
    file_name = raw_windows[0]['file']
    ds_name = file_name.split('-')[0]
    # filename = os.path.join(os.path.dirname(filename), ds_name + os.path.basename(filename))
    output_file = os.path.join(os.path.dirname(output_file), ds_name + '-' + os.path.basename(output_file))


    # udapi_doc = udapi.Document(filename)
    
    # # Write original Conllu without extra information
    # for _, mentions in udapi_doc.eid_to_entity.items():
    #     for m in mentions.mentions:
    #         m._other.clear()
    #         m._other._string = ''

    # with open(output_file, "w", encoding="utf-8") as f:
    #     write_data([udapi_doc], f)
    ...
    prev_max_ent = 0
    dir = 'mapper/docs'
    doc_order = write_docs_to_files(dir, filename)
    ...
    for doc_id, lines in docs.items():
        ...
        udapi_doc = udapi.Document(os.path.join(dir, doc_file_name))

        # Map text to Conllu
        for window in lines:
            if not "[MID]" in window['text']:
                window['text'] = window['text'] + "[MID]"
                
        lines_text = [tuple(map(str.strip, window['text'].split('[MID]'))) for window in lines] #text
        mapper = Mapper(udapi_doc, lines_text)
        mapper.map_outputs(prev_max_ent, os.path.join(dir, doc_file_name))
        if mapper.doc.coref_entities:
            prev_max_ent = max(list(map(lambda i: int(i.eid[1:]), mapper.doc.coref_entities)))
        ...
        with open(doc_gold, 'w') as f:
            f.write(''.join(doc_lines))
        
        print(mapper.doc.meta['docname']+'\n')
        os.system(f'python3 {s}corefud-scorer/corefud-scorer.py {doc_gold} {doc_score}')
        os.remove(doc_gold)
        os.remove(doc_score)
        ### SCORE DOCUMENTS ###
    ...
    os.system(f'python3 {s}tools/validate.py --level 2 --coref --lang {lang} {output_file}')
    os.system(f'python3 {s}corefud-scorer/corefud-scorer.py {filename} {output_file}')
    print(output_file)


# üst üste iki mention tahmin ediyorsa kısa olanı sil


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('-g', '--gold_conllu', default='mapper/dev.conllu', required=False, help='Gold conllu data to compare')
    parser.add_argument('-p', '--donor_conllu', default='mapper/dev.conllu', required=False, help='Predicted mentions conllu data for mention location')
    parser.add_argument('-o', '--output_path', default="mapper/out.conllu", required=False, help="output path")
    parser.add_argument('-l', '--llm_output_file', default='mapper/output.jsonl', required=False, help="LLM jsonl çıktısı")
    parser.add_argument('-f', '--filename', required=False, help="Name of the dataset without extension")
    parser.add_argument('-d', '--doc_id', required=False, help="doc_id for getting one doc from jsonl")
    parser.add_argument('-s', '--dataset_name', required=False, help="for example: tr_itcc")
    ...
    assert llm_output
    main(args.gold_conllu, args.output_path, llm_output)
