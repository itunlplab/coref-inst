...
def match_mentions(mention_words, pred_words):
    # Assume start and end are the same
    # Consider MWTs
    pred_words = mwt_serialize(pred_words)
    assert bool(pred_words)
    ...
    for gold_words, gold_head in mention_words:
        ...
        if pred_words[-1].is_empty() and len(pred_words) > 1:
            i=0
            while len(pred_words) > -i and not pred_words[i].is_empty():
                i-=1
                found_end = str(pred_words[i].address)
        else:
            found_end = str(pred_words[-1].address)
        
        # Direct match
        if gold_start_address == str(pred_words[0].address) and\
    ...
    if alternatives:
        if len(alternatives) == 1:
            ...
            return min(alternatives, key=lambda i: abs(i[0][0].ord - pred_words[0].ord)) # first word dist
    
    raise Exception(f"Not matched {pred_words}")
...
class Mapper:
    ...
    def map_outputs(self, prev_ent_counter):
        start_w_1 = 0
...
def main(filename, output_file, raw_windows):
    ...
    src_file_name = raw_windows[0]['file']
    ds_name = src_file_name.split('-')[0]
    ...
    for doc_id, lines in docs.items():
        ...
        mapper.map_outputs(prev_max_ent)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--doc', required=False, help="doc_id for getting one doc from jsonl")
