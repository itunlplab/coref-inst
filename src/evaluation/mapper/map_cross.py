...
def main(filename, output_file, raw_windows):
    ...
    #     os.path.dirname(filename), ds_name + os.path.basename(filename)
    # )
    ...
    for doc_id, lines in docs.items():
        ...
        os.system(
            (
                f'python3 {s}corefud-scorer/corefud-scorer.py '
                f'{doc_gold} {doc_score}'
            )
        )
        os.remove(doc_gold)
        os.remove(doc_score)
    ...
    os.system(
        f'python3 {s}corefud-scorer/corefud-scorer.py {filename} {output_file}'
    )
    print(output_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--donor_conllu', required=False,
