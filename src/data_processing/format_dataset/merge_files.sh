...
merge_jsonl_files() {
    ...
    if [ -d "$folder" ]; then
        # Merge train files
        cat "$folder"/*-train.jsonl >> "$output/train$extension.jsonl"
        
        # Merge dev files
        cat "$folder"/*-dev.jsonl >> "$output/dev$extension.jsonl"

        # Merge test
        cat "$folder"/*-test.jsonl >> "$output/test$extension.jsonl"
        
        # Merge test files
        cat "$folder"/*-dev2.jsonl >> "$output/dev2$extension.jsonl"
