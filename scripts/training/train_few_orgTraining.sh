...
for model in ${MODELS[@]}; do
    for ins_num in {1..5}; do
        for seed in ${SEEDS[@]}; do
            echo "Model: $model, Ins Num: $ins_num, Seed: $seed"
            XFORMERS_MORE_DETAILS=1 srun python ../../src/training/train_unified.py \
            --model_name $model \
            --dataset_name HFDS_5-shot \
            --ins_num $ins_num \
            --seed $seed
        done
    done
done
# old_ds/HFDS_train_few
